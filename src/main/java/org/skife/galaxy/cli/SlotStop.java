        ...
        try {
            ...
            for (SlotDescription slot : root.getSlots()) {
                if (slot.getId().toString().startsWith(slotId)) {
                    Action stop = Iterables.find(slot.getActions(), beanPropertyEquals("rel", "stop"));
                    Response r = http.prepareRequest(new RequestBuilder(stop.getMethod())
                                                         .setUrl(stop.getUri().toString())
